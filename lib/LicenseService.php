...
{
  protected function extractLicenseTextSection(string $text, string $startSectionTitle, string $endSectionTitle): string|null
  {
  ...
  protected function parseLicenses(string $licenseTextSection)
  {
    $licenses = [];
    $blocks = preg_split('/\n\d+\n/', $licenseTextSection);
  ...
  public function extractLicenses(string $filePath)
  {
    $parser = new Parser();
    $pdf = $parser->parseFile($filePath);
    $text = $pdf->getText();

    $licensesSection = $this->extractLicenseTextSection(
      $text,
    ...
    $licenses = $this->parseLicenses($licensesSection);
